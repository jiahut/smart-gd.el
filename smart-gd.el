...
(defun smart-gd--at-definition-via-lsp-p ()
  "Check if point is at a definition using LSP/xref.
This is more accurate than heuristics as it uses the language server."
  (condition-case nil
      (let* ((identifier (xref-backend-identifier-at-point (xref-find-backend)))
             (defs (when identifier
                     (xref-backend-definitions (xref-find-backend) identifier))))
        (when defs
          (let ((current-pos (point))
                (current-file (buffer-file-name)))
            ;; Check if any definition is at the current position
            (cl-some (lambda (def)
                       (let ((location (xref-item-location def)))
                         (when (xref-file-location-p location)
                           (and (string= (xref-file-location-file location) current-file)
...
(defun smart-gd--at-definition-p ()
  "Check if point is at a definition.
First tries LSP-based detection, falls back to heuristics."
  ;; Try LSP-based detection first (more accurate)
  (or (smart-gd--at-definition-via-lsp-p)
      ;; Fall back to heuristics for cases where LSP isn't available
      (save-excursion
        (beginning-of-line)
